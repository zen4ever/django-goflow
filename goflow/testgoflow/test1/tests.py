...
class Test(unittest.TestCase):
    ...
    def test_notif(self):
        ...
        workitem = ProcessInstance.objects.start('test2', user, model)
    ...
    def test_push_apps(self):
        user = User.objects.get(username='userA')
        workitem = ProcessInstance.objects.start('test2', user, DefaultAppModel.objects.create(comment='test_notif'))
    ...
    def test_forwd_nopush(self):
        ...
        workitem = ProcessInstance.objects.start('question_process', user, DefaultAppModel.objects.create(comment='test'))
        ...
        user.groups.add(B)
        lwi = WorkItem.objects.list_safe(user=user, notstatus='c', noauto=True)
